...
checkpt_file = 'pre_trained/cora/mod_cora{}.ckpt'.format(number)
...
with tf.Graph().as_default():
    ...
    logits,h_2 = model.inference(ftr_in, nb_classes, nb_nodes, is_train,
                                attn_drop, ffd_drop,
                                bias_mat=bias_in,
                                hid_units=hid_units, n_heads=n_heads,
                                residual=residual, activation=nonlinearity)
    hidden = tf.reshape(h_2, [-1, h_2.shape[2]])
    log_resh = tf.reshape(logits, [-1, nb_classes])
    lab_resh = tf.reshape(lbl_in, [-1, nb_classes])
    msk_resh = tf.reshape(msk_in, [-1])

    pred = tf.nn.softmax(log_resh)
    ...
    with tf.Session() as sess:
        ...
        while ts_step * batch_size < ts_size:
            ...
            loss_value_ts, acc_ts, pred_ts = sess.run([loss, accuracy, pred],
                feed_dict={
        ...
        np.save("/network/rit/lab/ceashpc/sharedata_shu/hidden/gat_{}_{}.npy".format(dataset, number), pred_ts)
        ...
        for p in range(500):
            ...
            while Bayets_step * batch_size < Bayets_size:
                if sparse:
                    ...
                    bbias = biases[ts_step * batch_size:(ts_step + 1) * batch_size]
                loss_value_Bayets, acc_Bayets, Baye_output, hidden_Baye = sess.run([loss, accuracy, log_resh, hidden],
                   feed_dict={
                       ftr_in: features[
                               Bayets_step * batch_size:(
                                                                    Bayets_step + 1) * batch_size],
                       bias_in: bbias,
                       lbl_in: y_test[
                               Bayets_step * batch_size:(
                                                                    Bayets_step + 1) * batch_size],
                       msk_in: test_mask[
                               Bayets_step * batch_size:(
                                                                    Bayets_step + 1) * batch_size],
                       is_train: False,
                       attn_drop: 0.6, ffd_drop: 0.4})
                Bayets_loss += loss_value_Bayets
                Bayets_acc += acc_Bayets
                Bayets_step += 1
            Baye_hidden.append(hidden_Baye)
            Baye_result.append(Baye_output)

        np.save("/network/rit/lab/ceashpc/sharedata_shu/hidden/hidden_gat_ori_feature_{}.npy".format(dataset),
                np.mean(Baye_hidden, axis=0))
        np.save("/network/rit/lab/ceashpc/sharedata_shu/hidden/Baye_gat_ori_result_{}.npy".format(dataset), Baye_result)
